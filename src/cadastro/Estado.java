...
public class Estado extends JDialog {
	...
	private void initComponents() {
		...
		actionPanel.setBounds(5, 275, 395, 35);
		...
		bodyPanel.setBounds(5, 70, 400, 200);
		getContentPane().add(bodyPanel);
		bodyPanel.setLayout(null);

		scrollPane = new JScrollPane();
		scrollPane.setBounds(5, 35, 390, 160);
	...
	private void saveButtonActionPerformed(ActionEvent evt) {
		...
		if (!codTextField.getText().trim().isEmpty()) {
			sql = "Update `" + util.getNameBD() + "`.`" + tabela + "` Set `"
					...
					+ "', `" + tableCols.get(2) + "`='"
					+ siglaTextField.getText().toUpperCase() + "', `user`='"
			...
			if (!nomeTextField.getText().trim().isEmpty()) {
				sql = "Insert Into `" + util.getNameBD() + "`.`" + tabela
						...
						+ "', '" + siglaTextField.getText().toUpperCase()
						+ "', '" + util.getUserSis() + "');";
				if (b.executarSQL(sql) > 0) {
					JOptionPane.showMessageDialog(null,
	...
	private void printButtonActionPerformed(ActionEvent evt) {
		try {
			...
			JasperDesign desenho = JRXmlLoader
					.load("relatorios/cadastro/Estado.jrxml");
			// compila o relatório
			...
			ResultSet rs = b.query("Select `" + tableCols.get(0)
					+ "` as codigo, `" + tableCols.get(1) + "` as nome, `"
					+ tableCols.get(2) + "` From `" + util.getNameBD() + "`.`"
					+ tabela + "` Where (`active`='1') order by "
					+ tableCols.get(0) + ";");
	...
	private void carregarRegistro(int cod) {
		if (cod >= 0) {
			...
			siglaTextField.setText((String) tabela.getValueAt(cod, 2));
		} else {
			JOptionPane
					.showMessageDialog(
							null,
	...
	private void carregarTabela(ResultSet rs) {
		try {
			...
			for (int i = 1; i < meta.getColumnCount() + 1; ++i) {
				...
				tmp = tmp.contains("State") ? "Estado" : tmp;
			...
			dadosTable.getColumnModel().getColumn(2).setPreferredWidth(50);
			if (dadosTable.getColumnCount() > 3) {
				util.ocultarColunas(dadosTable, 3,
